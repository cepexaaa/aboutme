...
public abstract class BaseTester<X, E extends Engine<X>> extends Tester {
    ...
    private void test(final Test test, final Consumer<Engine.Result<X>> check) {
        ...
        spoilers.forEach(spoiler -> {
            final String spoil = spoiler.spoil(test.unparsed(), test.expr, random());
            if (spoil.contains(":")) {
                System.out.println(spoiler.spoil(test.unparsed(), test.expr, random()));
                System.out.println("??? " + test.unparsed() + " -> " + spoil);
            }
            fullCheck.accept(parse(spoil));
        });
    }
